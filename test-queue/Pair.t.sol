...
contract PairTest is BaseTest {
    ...
    function deployPairFactoryGauge() public {
        ...
        staking = new TestStakingRewards(address(pair), address(oToken));
    ...
    function addGaugeAndBribeRewards() public {
        ...
        oToken.approve(address(gauge), PAIR_1);
        oToken.approve(address(xbribe), PAIR_1);
        oToken.approve(address(staking), PAIR_1);

        gauge.notifyRewardAmount(address(oToken), PAIR_1);
    ...
    function gaugeDistributeBasedOnVoting() public {
        ...
        oToken.approve(address(voter), PAIR_1);
    ...
    function gaugeClaimRewardsOwner3NextCycle() public {
        ...
        vm.warp(block.timestamp + 1);
        // uint256 earned = gauge.earned(address(oToken), address(owner3));
        address[] memory rewards = new address[](1);
        rewards[0] = address(oToken);
        owner3.getGaugeReward(address(gauge), address(owner3), rewards);
    ...
    function testGaugeClaimRewards3() public {
        ...
        pair.approve(address(gauge), PAIR_1);
        gauge.deposit(PAIR_1, 0);
        oToken.approve(address(gauge), oToken.balanceOf(address(owner)));
        gauge.notifyRewardAmount(address(oToken), oToken.balanceOf(address(owner)));
